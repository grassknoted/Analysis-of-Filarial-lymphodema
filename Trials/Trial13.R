...
#Generating Set 1
set1 <- subset(dermato_examinations, affected_nonaffected_limb == 2 & bep == 1)
...
set2 <- merge(a, c, by = c("patient_id"))
...
d <- subset(dermato_examinations, affected_nonaffected_limb == 1 & followup_code == 4 & bep == 1)
