#Comparison of Change in Volume for patients who have used compression and those who have not used compression using vol_change table
library(RMySQL)
library(dplyr)
...
for(i in 1:7000){
  iter1 <- subset(limb_vol_change, patient_limb_id == i & limb_code == 1)
  iter2 <- subset(limb_vol_change, patient_limb_id == i & limb_code == 2)
  ...
  if(nrow(comp1) > 0){
    ...
    if(nrow(iter1) > 0){
      ...
      for(j in 1:nrow(iter1))
        avg_comp1$avg_change[as.integer(iter1$followup_code[j])] <- avg_comp1$avg_change[as.integer(iter1$followup_code[j])] + iter1$vol_change[j]
    }
  
    #Average for Limb 2
    if(nrow(iter2) > 0){
      limb2_comp1 <- limb2_comp1 + 1
      for(j in 1:nrow(iter2))
        avg_comp1$avg_change[as.integer(iter2$followup_code[j])] <- avg_comp1$avg_change[as.integer(iter2$followup_code[j])] + iter2$vol_change[j]
    }
  }
  ...
  if(nrow(comp2) > 0){
    ...
    if(nrow(iter1) > 0){
      ...
      for(j in 1:nrow(iter1))
        avg_comp2$avg_change[as.integer(iter1$followup_code[j])] <- avg_comp2$avg_change[as.integer(iter1$followup_code[j])] + iter1$vol_change[j]
    }
    ...
    if(nrow(iter2) > 0){
      ...
      for(j in 1:nrow(iter2))
        avg_comp2$avg_change[as.integer(iter2$followup_code[j])] <- avg_comp2$avg_change[as.integer(iter2$followup_code[j])] + iter2$vol_change[j]
    }
  }
  ...
  if(nrow(comp3) > 0){
    ...
    if(nrow(iter1) > 0){
      ...
      for(j in 1:nrow(iter1))
        avg_comp3$avg_change[as.integer(iter1$followup_code[j])] <- avg_comp3$avg_change[as.integer(iter1$followup_code[j])] + iter1$vol_change[j]
    }
    ...
    if(nrow(iter2) > 0){
      ...
      for(j in 1:nrow(iter2))
        avg_comp3$avg_change[as.integer(iter2$followup_code[j])] <- avg_comp3$avg_change[as.integer(iter2$followup_code[j])] + iter2$vol_change[j]
    }
  }
}
